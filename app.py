...
st.markdown("""
    ...
    /* ขยายขนาดตัวหนังสือในช่องพิมพ์หลักให้อ่านง่าย สระวรรณยุกต์ไม่ซ้อนทับกัน */
    ...
    /* แผงฟังก์ชันมัลติมีเดียขอบล่างสุดสไตล์ ChatGPT */
    .custom-input-bar {
        background-color: #f0f4f9;
        padding: 8px 16px;
        border-radius: 20px;
...
st.markdown("ค้นหาข้อมูล เจาะลึกความรู้ หรือแนบไฟล์รูปภาพ/อัดเสียงพูดโต้ตอบได้ทุกภาษาในกล่องเดียว")
st.markdown("---")
...
if not api_key:
    ...
    with col_img:
        uploaded_image = st.file_uploader("🖼️ แนบไฟล์ภาพ", type=["jpg", "jpeg", "png"], key="img_box", label_visibility="visible")

    with col_voice:
        voice_recorder_data = st.audio_input("🎙️ อัดเสียงพูดสด", key="voice_box", label_visibility="visible")
    ...
    if user_prompt or voice_recorder_data:
        ...
        # 1. รูปภาพ (ถ้ามีการคลิกเลือกรูปภาพ 🖼️)
        if uploaded_image:
